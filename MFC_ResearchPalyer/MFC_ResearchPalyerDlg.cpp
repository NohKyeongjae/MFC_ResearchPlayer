...
void CMFC_ResearchPalyerDlg::OnBnClickedPathbutton()
{
	...
	TCHAR szFilters[] = _T("동영상 파일(*.mp4)|*.mp4||"); // 보기 텍스트란| 입력 텍스트란

	CFileDialog fileDlg(TRUE, _T("mp4"), _T("*.mp4"), //생겨나는 파일 속성
	...
	fileDlg.DoModal();
		...
		{
			CString pathName = fileDlg.GetPathName(); //선택한 파일의 경로
		...
		//버튼 클릭시 파일경로를 전달하여 파일 재생할 수 있도록 할 것******
		/*cvNamedWindow("new", CV_WINDOW_AUTOSIZE);
		CvCapture *capture = cvCreateFileCapture("C:\\Users\\노경재\\Videos\\aa.mp4");
		IplImage *frame;
		char c;
